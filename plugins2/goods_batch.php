...
if ($_REQUEST['act'] == 'add')
{
	...
	$smarty->assign('data_format_select', 'default_csv');
...
elseif ($_REQUEST['act'] == 'upload')
{
    ...
    elseif($_POST['data_cat'] == 'default_csv')
    {
		...
		// 显示的字段列表
		$smarty->assign('field_show', array('goods_sn' => true, 'goods_name' => true, 'goods_name_en' => true, 'brand_name' => true, 'shop_price' => true, 'goods_weight' => true, 'goods_spec' => true, 'goods_manufactory' => true, 'original_img' => true, 'goods_thumb' => true, 'goods_img' => true, 'preserve_time' => true, 'preserve_method' => true, 'cat_id' => true,));
...
//showr($_POST);return true;
        ...
        {//$counter = $counter + 1;
            ...
            foreach ($field_list AS $field)
            {
				...
				$duplicated = false;
				if ($_POST['goods_sn'][$value] != '')
				{
                        $sql = "SELECT COUNT(*) FROM " . $ecs->table('goods') . " WHERE goods_sn = '". $_POST['goods_sn'][$value]."'";
                        if ($db->getOne($sql) > 0)
                        {
							$duplicated = true;
						}
...
//print $field .": ". $field_arr[$field] . '<br><br>';
					// 特殊处理
			...
			//showr($field_arr);print '<br><br>';return true;
			//print count($field_arr);return true;
			if (isset($field_arr['duplicated']) && $field_arr['duplicated'] == 0)
			{		
