...
if (!$smarty->is_cached('index.dwt', $cache_id))
{
	...
	foreach($cat_info_list as $idx => $row){
		...
		foreach ($category_homepage_banner as $k => $v){
			if ($v['position'] == 'top'){
				$category_info[$idx]['top_banner'][$k]['src'] = $v['src'];
				$category_info[$idx]['top_banner'][$k]['url'] = $v['url'];
			}
			elseif ($v['position'] == 'right'){
				$category_info[$idx]['right_banner'][$k]['src'] = $v['src'];
				$category_info[$idx]['right_banner'][$k]['url'] = $v['url'];
			}
			elseif ($v['position'] == 'left'){
				$category_info[$idx]['left_banner'][$k]['src'] = $v['src'];
				$category_info[$idx]['left_banner'][$k]['url'] = $v['url'];
			}			
			elseif ($v['position'] == 'bottom'){
				$category_info[$idx]['bottom_banner'][$k]['src'] = $v['src'];
				$category_info[$idx]['bottom_banner'][$k]['url'] = $v['url'];
			}			
		}
	...
	foreach ($cat_info_list as $idx => $row){
		$cat_goods_info_list = assign_cat_goods($row["cat_id"], 30, 'wap', 'ORDER BY add_time DESC');
		//showr($cat_goods_info_list);
		$paged_result = index_get_panel_promo_array_paged($cat_goods_info_list['goods'],$perpage,$perrow);
...
foreach ($group_arry as $key => $value){
	...
	$item['short_name'] = ellipsis($item['short_name'], 5);
