...
elseif($action == 'oath')
{
	...
	if ($type == 'facebook'){
		session_start();
		require_once 'includes/website/config/facebook_config.php';
		...
		//保存来路URL，最后将返回
		$back_url = empty($_REQUEST['callblock']) ? "/index.php":$_REQUEST['callblock'];
		$_SESSION["back_url"] = $back_url;
		$loginUrl = $helper->getLoginUrl('http://warket.ca/includes/website/fb_callback.php', $permissions);
		//echo $loginUrl;
